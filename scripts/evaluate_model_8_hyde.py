...
MODEL_NAME = "model_8_hyde_hybrid_rerank"
MODEL_DESCRIPTION = (
    "Hybrid history selection, query rewriting, HyDE expansion, hybrid retrieval, fusion, and reranking. "
    "This model evaluates whether HyDE improves the full modular retrieval pipeline without answer generation."
)
...
def _append_unique_query(target: List[str], query: str) -> None:
...
def _build_hyde_generator(hyde_mode: str):
    if hyde_mode == "none":
        return None
    return HyDEQueryGenerator()
...
def select_history_for_model_8(
    question: str,
...
def generate_hyde_for_model_8(
    ...
    return hyde_query, hyde_used, hyde_failed, time.perf_counter() - started_at


def evaluate_model_8_hyde(
    ...
    candidate_k: int = 40,
    history_top_k: int = 4,
    output_path: str = "logs/eval_runs/model_8_hyde_legal_top5.json",
    ...
    hyde_generator = _build_hyde_generator(hyde_mode=hyde_mode)
    ...
    for index, sample in enumerate(data, start=1):
        ...
        rewritten_query, rewrite_failed, rewrite_latency_seconds = rewrite_query_for_model_8(
            question=question,
            selected_history=selected_history,
        ...
        hyde_query, hyde_used, hyde_failed, hyde_latency_seconds = generate_hyde_for_model_8(
        ...
        if include_original_query:
            _append_unique_query(query_variants, question)
        _append_unique_query(query_variants, query_used)
        if hyde_used:
            _append_unique_query(query_variants, hyde_query)
        elif hyde_failed:
            _append_unique_query(query_variants, query_used)
        ...
        fused_results = list(retrieval_state.get("retrieval_results", []) or [])
        ...
        total_hyde_latency += hyde_latency_seconds
        ...
        if hyde_used and not hyde_failed:
        ...
        sample_results.append(
            {
                ...
                "hyde_latency_seconds": hyde_latency_seconds,
    ...
    if evaluated_samples > 0:
        ...
        average_hyde_latency = total_hyde_latency / evaluated_samples
        ...
        hyde_success_rate = hyde_success_count / evaluated_samples
    ...
    report = {
        "metrics": {
            ...
            "avg_hyde_latency_seconds": average_hyde_latency,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate Model 8: HyDE + hybrid retrieval + reranking for multi-turn legal retrieval.",
    )
    parser.add_argument(
        "--eval-path",
    ...
    parser.add_argument(
        "--output-path",
        default="logs/eval_runs/model_8_hyde_legal_top5.json",
    )
    args = parser.parse_args()

    report = evaluate_model_8_hyde(
    ...
    print("===== MODEL 8 HYDE HYBRID RERANK =====")
    ...
    print(f"Avg HyDE latency (s): {metrics['avg_hyde_latency_seconds']:.4f}")
    ...
    print(f"HyDE success rate: {metrics['hyde_success_rate']:.4f}")
