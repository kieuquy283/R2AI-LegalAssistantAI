...
MODEL_NAME = "model_6_multi_query_hybrid"
MODEL_DESCRIPTION = (
    "Hybrid history selection, query rewriting, multi-query generation, and hybrid retrieval with fusion. "
    "No reranking, no answer generation."
)
...
def select_history_for_model_6(
    question: str,
...
def generate_multi_queries_for_model_6(
    rewritten_query: str,
    num_queries: int,
...
def evaluate_model_6_multi_query_hybrid(
    ...
    top_k: int = 10,
    history_top_k: int = 4,
    output_path: str = "logs/eval_runs/model_6_multi_query_hybrid_legal.json",
    ...
    for index, sample in enumerate(data, start=1):
        ...
        rewritten_query, rewrite_failed, rewrite_latency_seconds = rewrite_query_for_model_6(
            question=question,
            selected_history=selected_history,
        ...
        query_variants, multi_query_failed, multi_query_latency_seconds = (
            generate_multi_queries_for_model_6(
        ...
        final_results = list(final_state.get("retrieval_results", []) or [])
        retrieved_cids = _extract_cids_from_results(final_results)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate Model 6: multi-query hybrid retrieval over rewritten multi-turn queries.",
    )
    parser.add_argument(
        "--eval-path",
    ...
    parser.add_argument(
        "--output-path",
        default="logs/eval_runs/model_6_multi_query_hybrid_legal.json",
    )
    parser.add_argument(
        "--dense-weight",
        type=float,
    ...
    report = evaluate_model_6_multi_query_hybrid(
    ...
    print("===== MODEL 6 MULTI-QUERY HYBRID =====")
